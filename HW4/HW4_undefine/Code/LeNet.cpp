		...
		{ \
			...
			{ \
				...
				{ \
					...
					{ \
						Xtemp=Xtemp+input_name[FOR_l][(FOR_j+FOR_m/5)*_input_23_+(FOR_k+FOR_m%5)]*weight_name[FOR_i][FOR_l*25+FOR_m]; \
					} \
				} \
...
{ \
	for(int FOR_i=0; FOR_i < _output_len_; FOR_i++){ \
		...
		for(int FOR_j=0; FOR_j < _input_len_;FOR_j++){ \
			Xtemp=Xtemp+ input_name [FOR_j]* weight_name [FOR_i][FOR_j]; \
		} \
